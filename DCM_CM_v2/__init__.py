...
class Constants(BaseConstants):
    ...
    random_practice_choice_sit_2 = random.randint(1, num_CE)
...
def final_cal_practice(group):
 ...
 #  group1 = group.in_round(1)
    ...
    for p in group.in_round(1).get_players():
           ...
           group.sum_binding_sit_1_practice = p.CE_choice_practice* (p.CE_choice_practice == 1) + group.sum_binding_sit_1_practice
           group.sum_binding_sit_2_practice = int(p.CE_choice_practice * (p.CE_choice_practice == 2) / 2 )+ group.sum_binding_sit_2_practice
           group.sum_binding_sit_3_practice = int( p.CE_choice_practice * (p.CE_choice_practice == 3) / 3) + group.sum_binding_sit_3_practice
           group.id_player_groupA_practice = str(p.id_in_group)*(p.CE_choice_practice == 1)  + ',' +  group.id_player_groupA_practice
           group.id_player_groupB_practice = str(p.id_in_group)*(p.CE_choice_practice == 2)  + ',' +  group.id_player_groupB_practice
           group.id_player_groupC_practice = str(p.id_in_group)*(p.CE_choice_practice == 3)  + ',' +  group.id_player_groupC_practice


           group.list_pred_C_A_practice = str(p.CE_pract_pred_A)*(p.CE_choice_practice == 3) + ',' + group.list_pred_C_A_practice
           group.list_pred_C_B_practice = str(p.CE_pract_pred_B)*(p.CE_choice_practice == 3) + ',' + group.list_pred_C_B_practice
           group.list_pred_C_C_practice = str(p.CE_pract_pred_C)*(p.CE_choice_practice == 3) + ',' + group.list_pred_C_C_practice

           group.list_pred_A_A_practice =  str(p.CE_pract_pred_A)*(p.CE_choice_practice == 1) + ',' + group.list_pred_A_A_practice
           group.list_pred_A_B_practice = str(p.CE_pract_pred_B)*(p.CE_choice_practice == 1) + ',' + group.list_pred_A_B_practice
           group.list_pred_A_C_practice = str(p.CE_pract_pred_C)*(p.CE_choice_practice == 1) + ',' + group.list_pred_A_C_practice

           group.list_pred_B_A_practice = str(p.CE_pract_pred_A)*(p.CE_choice_practice == 2) + ',' + group.list_pred_B_A_practice
           group.list_pred_B_B_practice = str(p.CE_pract_pred_B)*(p.CE_choice_practice == 2) + ',' + group.list_pred_B_B_practice
           group.list_pred_B_C_practice = str(p.CE_pract_pred_C)*(p.CE_choice_practice == 2) + ',' + group.list_pred_B_C_practice
...
def payoffs_practice(replace_pred, player):
    own_prediction_A=  player.CE_pract_pred_A
    own_prediction_B = player.CE_pract_pred_B
    own_prediction_C = player.CE_pract_pred_C
    if replace_pred == 0:
           Pred_A = own_prediction_A/ (player.participant.num_players - 1)
           Pred_B = own_prediction_B/ (player.participant.num_players - 1)
           Pred_C = own_prediction_C/ (player.participant.num_players - 1)
        ...
        if player.participant.group_pre_cont_practice ==1 :
           payoffs = dict( error = True)

        else:
           Pred_A = ((player.participant.group_pre_A_sum_practice -  own_prediction_A)/
                  (player.participant.group_pre_cont_practice -1))/ (player.participant.num_players - 1)
           Pred_B = ((player.participant.group_pre_B_sum_practice -  own_prediction_B)/
                  (player.participant.group_pre_cont_practice -1))/ (player.participant.num_players - 1)

           Pred_C = ((player.participant.group_pre_C_sum_practice -  own_prediction_C)/
                  (player.participant.group_pre_cont_practice -1))/ (player.participant.num_players - 1)
...
class Practice_results_no_replace_1(Page):
    @staticmethod
    def is_displayed(player: Player):
        return (player.round_number == 1 and player.participant.replace_predictions_practice ==0)
...
class Practice_results_replace_1(Page):
    @staticmethod
    def is_displayed(player: Player):
        return (player.round_number == 1 and player.participant.replace_predictions_practice ==1)
...
class Practice_results_binding_alternative_payoff_replaced(Page):
    @staticmethod
    def is_displayed(player: Player):
        return (player.round_number == 1 and player.participant.replace_predictions_practice ==0)
...
class Practice_results(Page):
    ...
    def live_method(player: Player, data):
        if data['clicked_button'] == 1:
            choices_str = player.participant.list_choices_A_practice + ' ' + player.participant.list_choices_B_practice  + ' ' + player.participant.list_choices_C_practice
            list_choices = choices_str.split()

            player.participant.random_choice = random.choice(list_choices)
            random_choice = player.participant.random_choice

            payoff_set = payoffs(player.participant.replace_predictions_practice, player)
            if payoff_set['error'] == False:
                ...
                response = dict(replace_pred=player.participant.replace_predictions_practice, payoff_a=payoff_set['payoff_a'],
                                payoff_b=payoff_set['payoff_b'], payoff_c=payoff_set['payoff_c'],
                                finalpayoff=player.payoff,
                                pred_a=payoff_set['pred_a'], pred_b=payoff_set['pred_b'], pred_c=payoff_set['pred_c'],
                                round=1, choices=' '.join(list_choices))
            else:
                player.payoff = 99999
                response = dict(replace_pred=player.participant.replace_predictions_practice, payoff_a="Error",
...
class Results(Page):
    ...
    def live_method(player: Player, data):
        if data['clicked_button'] == 1:
            ...
            payoff_set = payoffs(player.participant.replace_predictions, player)
            if  payoff_set['error'] == False :
                if random_choice == 'A':
                   player.payoff = payoff_set['payoff_a']
                else:
                   if random_choice == 'B':
                       ...
                       player.payoff = payoff_set['payoff_c']
                response = dict(replace_pred=player.participant.replace_predictions, payoff_a=payoff_set['payoff_a'],
                            payoff_b=payoff_set['payoff_b'], payoff_c=payoff_set['payoff_c'], finalpayoff=player.payoff,
                            pred_a=payoff_set['pred_a'],  pred_b=payoff_set['pred_b'], pred_c=payoff_set['pred_c'],
                            round=1, choices = ' '.join(list_choices))
            else:
                player.payoff = 99999
                response = dict(replace_pred=player.participant.replace_predictions, payoff_a="Error",
                            payoff_b="Error", payoff_c="Error", finalpayoff=player.payoff,
                            pred_a="Error",  pred_b="Error", pred_c="Error",
                            round=1,choices = ' '.join(list_choices))
...
Practice_results_binding_alternative_payoff_replaced,
